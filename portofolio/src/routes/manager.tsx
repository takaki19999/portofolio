...
import { deleteActivity, getActivity, resetActivity, type ActivityRecord } from "@/lib/activity";
...
function AppCell({ record }: { record: ActivityRecord }) {
  if (record.downloadPercent !== null) {
    return <div className="min-w-28"><div className="mb-1 flex justify-between text-xs text-muted-foreground"><span>Downloading</span><span>{record.downloadPercent}%</span></div><div className="h-1.5 overflow-hidden rounded-full bg-secondary"><div className="h-full bg-accent transition-all" style={{ width: `${record.downloadPercent}%` }} /></div></div>;
  }
  return <span className={`rounded-full px-2 py-1 text-xs ${record.appStatus === "clicked" ? "bg-accent/10 text-accent" : "bg-secondary text-muted-foreground"}`}>{record.appStatus === "clicked" ? "Clicked" : "-"}</span>;
}
...
function Manager() {
  ...
  async function initializeData() {
    if (!window.confirm("Initialize all activity data? This permanently deletes every visitor record and click count.")) return;
    setInitializing(true);
    try {
      await resetActivity();
      known.current.clear();
      setRecords([]);
      toast.success("Activity data initialized");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Could not initialize activity data");
    } finally {
  ...
  return <main className="min-h-screen bg-background px-5 py-8 text-foreground"><Toaster richColors />
    <div className="mx-auto max-w-7xl">
      <header className="mb-8 flex items-center justify-between gap-4"><div><p className="font-mono text-xs uppercase tracking-widest text-accent">Portfolio control center</p><h1 className="mt-2 font-display text-3xl font-bold">Manager</h1><p className="mt-1 text-sm text-muted-foreground">One live row per IP address. Presence expires after 45 seconds without a heartbeat.</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => void enableNotifications()}><Bell className="mr-2 h-4 w-4" />Enable system notifications</Button><Button variant="outline" onClick={() => void refresh()}><RefreshCw className="mr-2 h-4 w-4" />Refresh</Button><Button variant="destructive" disabled={initializing} onClick={() => void initializeData()}><Trash2 className="mr-2 h-4 w-4" />{initializing ? "Initializing…" : "Initialize data"}</Button><Button asChild variant="hero"><Link to="/">View site</Link></Button></div></header>
